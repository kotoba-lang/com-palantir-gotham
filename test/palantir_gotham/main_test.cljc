(ns palantir_gotham.main-test
  "Contract + behavioral test for the palantir_gotham-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "palantir_gotham-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'palantir_gotham.main-test)]
