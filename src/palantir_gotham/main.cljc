(ns palantir_gotham.main
  "Kotodama WASM entrypoint for the Palantir_gotham clean-room actor (L4) — Clojure port.
  ...
  facts (`palantir_gotham.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "palantir_gotham")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"palantir_gotham.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "palantir_gotham-compat" :tier tier :entities entities} 200])
...
(defn start! [] :palantir_gotham-compat/ready)
